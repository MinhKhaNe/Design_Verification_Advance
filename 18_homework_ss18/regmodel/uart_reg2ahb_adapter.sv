class uart_reg2ahb_adapter extends uvm_reg_adapter;
  ...
  virtual function uvm_sequence_item reg2bus(const ref uvm_reg_bus_op rw);
    ...
    ahb.addr      = rw.addr;
    ahb.data      = rw.data;
    `uvm_info(get_type_name(), $sformatf("reg2bus: address = 0x%0h data = 0x%0h kind = %0s", ahb.addr, ahb.data, ahb.xact_type), UVM_HIGH)
    return ahb;
  endfunction 
  ...
  virtual function void bus2reg(uvm_sequence_item bus_item, ref uvm_reg_bus_op rw);
    ...
    `uvm_fatal(get_type_name(), "FAILED to cast bust_item to ahb transaction")
