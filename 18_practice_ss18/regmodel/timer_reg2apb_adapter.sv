class timer_reg2apb_adapter extends uvm_reg_adapter;
  `uvm_object_utils(timer_reg2apb_adapter)

  function new(string name="timer_reg2apb_adapter");
    super.new(name);
  ...
  virtual function uvm_sequence_item reg2bus(const ref uvm_reg_bus_op rw);
    ...
    apb.addr = rw.addr;
    apb.data = rw.data;
    `uvm_info(get_type_name(),$sformatf("reg2bus: addr=0x%0h data=0x%0h kind=%0s",apb.addr, apb.data, apb.xact_type.name()),UVM_HIGH)
    return apb;
  endfunction 
  ...
  virtual function void bus2reg(uvm_sequence_item bus_item, ref uvm_reg_bus_op rw);
    ...
    if(!$cast(apb,bus_item))
      `uvm_fatal(get_type_name(),"Failed to cast bus_item to apb transaction")
