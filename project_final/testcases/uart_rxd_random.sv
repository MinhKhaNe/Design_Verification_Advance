class uart_rxd_random extends base_test;
  `uvm_component_utils(uart_rxd_random)

  ahb_rxd_write_sequence  ahb_write;
  ahb_rxd_read_sequence   ahb_read;
  uart_rxd_write_sequence uart_write;
  
  int baud_rate_a[7] = '{2400, 4800, 9600, 19200, 38400, 76800, 115200};

  function new(string name = "uart_rxd_random", uvm_component parent);
        ...
        for(int sb = 1; sb <= 2; sb++) begin
          for(int br = 0; br < 7; br++) begin
            for(int m = 0; m < 2; m++) begin
              ...
              ahb_write     = ahb_rxd_write_sequence::type_id::create("ahb_write", this);
              ahb_write.cfg = cfg;
