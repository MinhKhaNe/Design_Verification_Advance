class uart_rxd_parity_status extends base_test;
  `uvm_component_utils(uart_rxd_parity_status)

  ahb_rxd_write_parity_status_sequence  ahb_write;
  ahb_rxd_read_parity_status_sequence   ahb_read;
  uart_rxd_write_sequence               uart_write;
  
  int baud_rate_a[7] = '{2400, 4800, 9600, 19200, 38400, 76800, 115200};

  function new(string name = "uart_rxd_parity_status", uvm_component parent);
        ...
        for(int sb = 1; sb <= 2; sb++) begin
          for(int br = 0; br < 7; br++) begin
            for(int m = 0; m < 2; m++) begin
              ...
              ahb_read      = ahb_rxd_read_parity_status_sequence::type_id::create("ahb_read", this);
              ahb_read.cfg  = cfg;
              ahb_read.exp  = data;
