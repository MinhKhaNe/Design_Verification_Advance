  ...
  function void write_interrupt(interrupt_transaction trans);
    ...
    case(int_state)
      ...
      INT_FULL_DEASSERT: begin
        if(!trans.interrupt) begin
          `uvm_info(get_type_name(), $sformatf("===== Interrupt is deasserted ====="), UVM_LOW)
          int_state = INT_FULL_ASSERT;
        end
        else begin
      ...
      INT_EMPTY_ASSERT: begin
        if(trans.interrupt) begin
          ...
          `uvm_error(get_type_name(), $sformatf("===== FAILED!!! Interrupt should be asserted ====="))
        end
      end
