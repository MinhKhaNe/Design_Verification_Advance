class data_frame_x13_chk_sequence extends uvm_sequence;
  `uvm_object_utils(data_frame_x13_chk_sequence)
  ...
  function new(string name = "data_frame_x13_chk_sequence");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    //for(int i = 0; i < 3; i++) begin
      ...
      if(cfg.data_width == 5) begin
        write_ahb(LCR, 32'h20);
      end
      else if(cfg.data_width == 6) begin
        write_ahb(LCR, 32'h21);
      end
      else if(cfg.data_width == 7) begin
        write_ahb(LCR, 32'h22);
      end
      else begin
        write_ahb(LCR, 32'h23);
      end
      write_ahb(TBR, wdata);
    //end
  endtask
