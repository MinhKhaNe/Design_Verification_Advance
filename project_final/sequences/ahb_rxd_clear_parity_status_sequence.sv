class ahb_rxd_clear_parity_status_sequence extends uvm_sequence #(ahb_transaction);
  `uvm_object_utils(ahb_rxd_clear_parity_status_sequence)
  ...
  function new(string name = "ahb_rxd_clear_parity_status_sequence");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if(data[3] != 1'b1) begin
      ...
      if(data[4] != 1) begin
        ...
        `uvm_info(get_type_name(), $sformatf("\n===== Parity Status Error Triggered ====="), UVM_LOW)
      end
    end

    write_ahb(FSR, 32'h10);
    
    regmodel.FSR.read(status, data);
    if(data[4] != 0) begin
      `uvm_error(get_type_name(), $sformatf("\n===== FAILED!!! Parity Error Status is not cleared ====="))
    end
    else begin
      `uvm_info(get_type_name(), $sformatf("\n===== Parity Status Error is cleared ====="), UVM_LOW)
    end
